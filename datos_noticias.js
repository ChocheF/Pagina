...
const filtro_a_usar = ".col-xs-12 col-sm-12 col-md-12 pad-none";
...
getDataUrl(ruta, filtro_a_usar)
  .then(function (respuesta) {
    ...
    let titulo = $(articulos[10]).find("article div.col-md-9 col-sm-9 col-xs-9 a").text();
    let fecha = $(articulos[10]).find("article div.col-md-9 col-sm-9 col-xs-9 .fecha aright").text().trim();
    ...
    for (let indice = 0; indice < articulos.length; indice++) {
      ...
      let fecha = $(articulos[indice])
        .find("article div.col-md-9 col-sm-9 col-xs-9 .fecha aright") // Busca el elemento
